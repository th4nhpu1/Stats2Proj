...
**Research question:** In what sense is the $L_2$ penalty we apply to the output
weights of our fixed ReLU feature model (Problem 4C) the same mechanism as
"weight decay" in neural-network training, and where does that equivalence break
down?

| Claim | Exact primary source and locator | What it establishes | What it does not establish |
|---|---|---|---|
| Adding a term proportional to $\sum_i w_i^2$ to the training cost improves generalization by penalizing large weights, and its effect can be characterized through the reduction in the effective number of parameters. | Krogh and Hertz [-@krogh1991], *NeurIPS 4*, Eq. (1)–(2) and the "effective number of parameters" analysis. | That an $L_2$ cost term shrinks weights toward zero and yields a measurable generalization improvement in a feedforward network; this is the same additive penalty used in ridge. | That the shrinkage produces exact zeros, that it applies to input-to-hidden weights specifically, or that the equivalence survives adaptive optimizers. |
| **(Methodological)** With adaptive gradient methods, adding $L_2$ to the loss is *not* equivalent to decaying weights; the decay must be decoupled from the gradient update. | Loshchilov and Hutter [-@loshchilov2019], ICLR 2019, Algorithm 2 (AdamW) and §2–3. | That "weight decay" and "$L_2$ penalty" name different procedures once the optimizer rescales gradients per-coordinate; prescribes decoupling as the fix. | That either variant induces sparsity, or that any conclusion transfers to our convex, closed-form `glmnet` setting, which uses neither Adam nor SGD. |
